...
data_sales = pd.read_excel("c:\\users\\catcry\\desktop\\iec\data\\Sales.xlsx")
...
clustering_Russia =  AgglomerativeClustering (n_clusters=n_c ).fit(Russia) 
clustering_China =  AgglomerativeClustering (n_clusters=n_c ).fit(China)
...
clustering_Spain =  AgglomerativeClustering (n_clusters=n_c ).fit(Spain)
clustering_Australia =  AgglomerativeClustering (n_clusters=n_c ).fit(Australia)
clustering_India =  AgglomerativeClustering ( n_clusters=n_c).fit(India)
clustering_SaudiArabia =  AgglomerativeClustering ( n_clusters=n_c).fit(SaudiArabia)
...
clustering_Germany =  AgglomerativeClustering ( n_clusters=n_c).fit(Germany)



clustering_Russia_vars = np.zeros([n_c,1])
clustering_China_vars = np.zeros([n_c,1])
...
clustering_Australia_vars = np.zeros([n_c,1])
clustering_India_vars =np.zeros([n_c,1])
clustering_SaudiArabia_vars = np.zeros([n_c,1])
clustering_Mexico_vars = np.zeros([n_c,1])
clustering_Britain_vars = np.zeros([n_c,1])
clustering_Italy_vars = np.zeros([n_c,1])
clustering_Germany_vars = np.zeros([n_c,1])
  
sumz = np.zeros([18,1])
  
for i in range(0,n_c):
    clustering_rus_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_Russia.labels_,17)),[len(Russia),17]) ==i, Russia))
sumz[0] = np.average(clustering_Russia_vars)

for i in range(0,n_c):
    clustering_chn_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_China.labels_,17)),[len(China),17]) ==i, China))
sumz[1] = np.average(clustering_China_vars)
...
for i in range(0,n_c):
    clustering_spain_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_Spain.labels_,17)),[len(Spain),17]) ==i, Spain))
sumz[10] = np.average(clustering_Spain_vars) 
    
for i in range(0,n_c):
    clustering_aus_vars[i] = len(np.extract(np.reshape((np.repeat(clustering_Australia.labels_,17)),[len(Australia),17]) ==i, Australia))
...
for i in range(0,n_c):
    clustering_Arabs_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_SaudiArabia.labels_,17)),[len(SaudiArabia),17]) ==i, SaudiArabia))
sumz[13] = np.average(clustering_SaudiArabia_vars)
...
sumz[15] = np.average(clustering_Britain_vars)
...
for i in range(0,n_c):
    clustering_german_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_Germany.labels_,17)),[len(Germany),17]) ==i, Germany))
sumz[17] =   np.average(clustering_Germany_vars)  
...
#    
 ...
 countrystg = input('Enter A country Name: ')
for i in range (0,n_c): 
    country = eval (countrystg)
    cluz = eval ('clustering_'+countrystg )
    lablz = eval('clustering_'+countrystg).labels_
   # c_name = eval(countrystg+'_cluster_%d'%i)
    globals()[countrystg+'_cluster_%d'%i] = np.extract (np.reshape((np.repeat(lablz,17)),[len(country),17]) ==i, country)
    globals()[countrystg+'_cluster_%d'%i] = np.reshape (globals()[countrystg+'_cluster_%d'%i], [len(globals()[countrystg+'_cluster_%d'%i])//17,17])        
