...
for i in range(0,n_c):
    clustering_rus_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_rus.labels_,15)),[len(Russia),15]) ==i, Russia))
sumz[0] = np.average(clustering_rus_vars)

for i in range(0,n_c):
    clustering_chn_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_chn.labels_,15)),[len(China),15]) ==i, China))
sumz[1] = np.average(clustering_chn_vars)

for i in range(0,n_c):
    clustering_Iran_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_Iran.labels_,15)),[len(Iran),15]) ==i, Iran))
sumz[2] = np.average(clustering_Iran_vars)
    
for i in range(0,n_c):
    clustering_brazil_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_brazil.labels_,15)),[len(Brazil),15]) ==i, Brazil))
sumz[3] = np.average(clustering_brazil_vars)
    
for i in range(0,n_c):
    clustering_Turkey_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_Turkey.labels_,15)),[len(Turkey),15]) ==i, Turkey))
sumz[4] = np.average(clustering_Turkey_vars)
    
for i in range(0,n_c):
    clustering_Japan_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_Japan.labels_,15)),[len(Japan),15]) ==i, Japan))
sumz[5] = np.average(clustering_Japan_vars)
    
for i in range(0,n_c):
    clustering_USA_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_USA.labels_,15)),[len(USA),15]) ==i, USA))
sumz[6] = np.average(clustering_USA_vars) 
    
for i in range(0,n_c):
    clustering_canada_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_canada.labels_,15)),[len(Canada),15]) ==i, Canada))
sumz[7] = np.average(clustering_canada_vars)        
    
for i in range(0,n_c):
    clustering_france_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_france.labels_,15)),[len(France),15]) ==i, France))
sumz[8] = np.average(clustering_france_vars)
    
for i in range(0,n_c):
    clustering_iraq_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_iraq.labels_,15)),[len(Iraq),15]) ==i, Iraq))
sumz[9] = np.average(clustering_iraq_vars)
    
for i in range(0,n_c):
    clustering_spain_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_spania.labels_,15)),[len(Spain),15]) ==i, Spain))
sumz[10] = np.average(clustering_spain_vars) 
    
for i in range(0,n_c):
    clustering_aus_vars[i] = len(np.extract(np.reshape((np.repeat(clustering_aus.labels_,15)),[len(Australia),15]) ==i, Australia))
sumz[11] = np.average(clustering_aus_vars)    
    
for i in range(0,n_c):
    clustering_india_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_india.labels_,15)),[len(India),15]) ==i, India))
sumz[12] = np.average(clustering_india_vars)
    
for i in range(0,n_c):
    clustering_Arabs_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_Arabs.labels_,15)),[len(SaudiArabia),15]) ==i, SaudiArabia))
sumz[13] = np.average(clustering_Arabs_vars)
    
for i in range(0,n_c):
    clustering_Mexico_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_Mexico.labels_,15)),[len(Mexico),15]) ==i, Mexico))
sumz[14] = np.average(clustering_Mexico_vars)

    
for i in range(0,n_c):
    clustering_brit_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_Britain.labels_,15)),[len(Britain),15]) ==i, Britain))
sumz[15] = np.average(clustering_brit_vars)
    
for i in range(0,n_c):
    clustering_italy_vars[i] = len(np.extract(np.reshape((np.repeat(clustering_italy.labels_,15)),[len(Italy),15]) ==i, Italy))
sumz[16] =np.average(clustering_italy_vars)

for i in range(0,n_c):
    clustering_german_vars[i] = len(np.extract (np.reshape((np.repeat(clustering_german.labels_,15)),[len(Germany),15]) ==i, Germany))
